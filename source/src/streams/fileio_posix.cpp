...
#include <fcntl.h> // OPEN TODO:: windows only>???
#ifdef _WIN32
#include <io.h>
#endif
...
/// </remarks>
bool _open_fsb_str(_filestream_callback *callback, const utility::char_t* filename, std::ios_base::openmode mode, int prot)
{
...
#ifdef _WIN32
        //auto bytes_written = pwrite(fInfo->m_handle, ptr, count, abs_position); -- mrkkrj
        off_t saved_pos = lseek(fInfo->m_handle, 0, SEEK_CUR);

        lseek(fInfo->m_handle, abs_position, SEEK_SET);
        auto bytes_written = write(fInfo->m_handle, ptr, count);

        lseek(fInfo->m_handle, saved_pos, SEEK_SET);
#else
...
size_t _read_file_async(Concurrency::streams::details::_file_info_impl *fInfo, Concurrency::streams::details::_filestream_callback *callback, void *ptr, size_t count, size_t offset)
{
    ...
    {
#ifdef _WIN32
        //auto bytes_read = pread(fInfo->m_handle, ptr, count, offset); -- mrkkrj
        off_t saved_pos = lseek(fInfo->m_handle, 0, SEEK_CUR);
