...
public:
    ...
    void upgrade_to_ssl(const std::function<void(asio::ssl::context&)>& ssl_context_callback)
    {
        ...
        m_ssl_stream = utility::details::make_unique<asio::ssl::stream<asio::ip::tcp::socket &>>(m_socket, ssl_context);
    }
#endif
...
private:
    ...
    static void start_epoch_interval(const std::shared_ptr<asio_connection_pool>& pool)
    {
        ...
        pool->m_pool_epoch_timer.expires_from_now(std::chrono::seconds(30));
        pool->m_pool_epoch_timer.async_wait([weak_pool](const std::error_code& ec)
        {
            if (ec)
                return;
    ...
    public:
        ...
        void start_proxy_connect()
        {
            ...
            client->m_resolver.async_resolve(query, 
                std::bind(&ssl_proxy_tunnel::handle_resolve, shared_from_this(), 
                    std::placeholders::_1 /*asio::placeholders::error*/, 
                    std::placeholders::_2 /*asio::placeholders::iterator*/));
        }

    private: 
        void handle_resolve(const std::error_code& ec, tcp::resolver::iterator endpoints)
        {
            if (ec)
            {
            ...
            else
            {
                ...
                m_context->m_connection->async_connect(endpoint, 
                    std::bind(&ssl_proxy_tunnel::handle_tcp_connect, shared_from_this(), std::placeholders::_1 /*asio::placeholders::error*/, ++endpoints));
            }
        }

        void handle_tcp_connect(const std::error_code& ec, tcp::resolver::iterator endpoints)
        {
            if (!ec)
            {
                m_context->m_timer.reset();
                m_context->m_connection->async_write(m_request, 
                    std::bind(&ssl_proxy_tunnel::handle_write_request, shared_from_this(), std::placeholders::_1 /*asio::placeholders::error*/));
            }
        ...
        void handle_write_request(const std::error_code& err)
        {
            ...
            {
                m_context->m_timer.reset();
                m_context->m_connection->async_read_until(m_response, CRLF + CRLF, 
                    std::bind(&ssl_proxy_tunnel::handle_status_line, shared_from_this(), std::placeholders::_1 /*asio::placeholders::error*/));
            }
            else
            {
        ...
        void handle_status_line(const std::error_code& ec)
        {
            ...
            else
            {
                ...
                const bool socket_was_closed((asio::error::eof == ec)
    ...
    void start_request()
    {
        ...
        auto start_http_request_flow = [proxy_type, proxy_host, proxy_port](std::shared_ptr<asio_context> ctx)
        {
            ...
            else
            {
                ...
                client->m_resolver.async_resolve(query, 
                    std::bind(&asio_context::handle_resolve, ctx, 
                        std::placeholders::_1 /*asio::placeholders::error*/,
                        std::placeholders::_2 /*asio::placeholders::iterator*/));
            }
...
private:
    ...
    void report_error(const std::string &message, const std::error_code &ec, httpclient_errorcode_context context = httpclient_errorcode_context::none)
    {
    ...
    void handle_connect(const std::error_code& ec, tcp::resolver::iterator endpoints)
    {
        m_timer.reset();
        if (!ec)
        {
        ...
        else
        {
            ...
            auto endpoint = *endpoints;
            m_connection->async_connect(endpoint, std::bind(&asio_context::handle_connect, shared_from_this(), std::placeholders::_1 /*asio::placeholders::error*/, ++endpoints));
        }
    }
    ...
    void handle_resolve(const std::error_code& ec, tcp::resolver::iterator endpoints)
    {
        ...
        else
        {
            m_timer.reset();
            auto endpoint = *endpoints;
            m_connection->async_connect(endpoint, 
                std::bind(&asio_context::handle_connect, shared_from_this(), std::placeholders::_1 /*asio::placeholders::error*/, ++endpoints));
        }
    }
    ...
    void write_request()
    {
        ...
        if (m_connection->is_ssl() && !m_connection->is_reused())
        {
            ...
            m_connection->async_handshake(asio::ssl::stream_base::client,
                                          m_http_client->client_config(),
                                          utility::conversions::to_utf8string(m_http_client->base_uri().host()),
                                          std::bind(&asio_context::handle_handshake, shared_from_this(), std::placeholders::_1 /*asio::placeholders::error*/),
    ...
    bool handle_cert_verification(bool preverified, asio::ssl::verify_context &verifyCtx)
    {
        ...
        asio::ssl::rfc2818_verification rfc2818(host);
    ...
    void handle_write_chunked_body(const std::error_code& ec)
    {
        ...
        uint8_t *buf = asio::buffer_cast<uint8_t *>(m_body_buf.prepare(chunkSize + http::details::chunked_encoding::additional_encoding_space));
        ...
        readbuf.getn(buf + http::details::chunked_encoding::data_offset, chunkSize).then([this_request, buf, chunkSize](pplx::task<size_t> op)
        {
            ...
            if (readSize != 0)
            {
                this_request->m_connection->async_write(this_request->m_body_buf,
                                                        std::bind(&asio_context::handle_write_chunked_body, this_request, std::placeholders::_1 /*asio::placeholders::error*/));
            }
            else
            {
                this_request->m_connection->async_write(this_request->m_body_buf,
                                                        std::bind(&asio_context::handle_write_body, this_request, std::placeholders::_1 /*asio::placeholders::error*/));
            }
        });
    }
    ...
    void handle_write_large_body(const std::error_code& ec)
    {
        ...
        auto readbuf = _get_readbuffer();
        readbuf.getn(asio::buffer_cast<uint8_t *>(m_body_buf.prepare(readSize)), readSize).then([this_request](pplx::task<size_t> op)
        {
            try
            {
                ...
                this_request->m_connection->async_write(this_request->m_body_buf, 
                    std::bind(&asio_context::handle_write_large_body, this_request, std::placeholders::_1 /*asio::placeholders::error*/));
            }
            catch (...)
            {
                this_request->report_exception(std::current_exception());
    ...
    void handle_write_body(const std::error_code& ec)
    {
        if (!ec)
        {
            ...
            m_connection->async_read_until(m_body_buf, CRLF + CRLF, 
                std::bind(&asio_context::handle_status_line, shared_from_this(), std::placeholders::_1 /*asio::placeholders::error*/)); // OPEN TODO::: ????
        }
        else
        {
        ...
        else
        {
            if (!needChunked)
            {
                async_read_until_buffersize(static_cast<size_t>(std::min(m_content_length, static_cast<uint64_t>(m_http_client->client_config().chunksize()))),
                                            std::bind(&asio_context::handle_read_content, shared_from_this(), std::placeholders::_1 /*asio::placeholders::error*/));
            }
            else
            {
                m_connection->async_read_until(m_body_buf, CRLF, std::bind(&asio_context::handle_chunk_header, shared_from_this(), std::placeholders::_1 /*asio::placeholders::error*/));
            }
        }
    }
    ...
    void handle_chunk_header(const std::error_code& ec)
    {
        if (!ec)
        {
            ...
            if (octetLine.fail())
            {
                report_error("Invalid chunked response header", std::error_code(), httpclient_errorcode_context::readbody);
            }
            else
            {
                async_read_until_buffersize(octets + CRLF.size(), // + 2 for crlf
                                            std::bind(&asio_context::handle_chunk, shared_from_this(), std::placeholders::_1 /*asio::placeholders::error*/, octets));
            }
        }
        else
        {
    ...
    void handle_chunk(const std::error_code& ec, int to_read)
    {
        if (!ec)
        {
            ...
            else
            {
                ...
                if(m_decompressor)
                {
                    auto decompressed = m_decompressor->decompress(asio::buffer_cast<const uint8_t *>(m_body_buf.data()), to_read);
                    ...
                    if (decompressed.empty())
                    {
                        ...
                        m_connection->async_read_until(m_body_buf, CRLF, std::bind(&asio_context::handle_chunk_header, this_request, std::placeholders::_1 /*asio::placeholders::error*/));
                    }
                    else
                    {
                            ...
                            .then([this_request, to_read, shared_decompressed](pplx::task<size_t> op)
                        {
                            try
                            {
                                ...
                                this_request->m_connection->async_read_until(this_request->m_body_buf, CRLF, std::bind(&asio_context::handle_chunk_header, this_request, std::placeholders::_1 /*asio::placeholders::error*/));
                            }
                            catch (...)
                            {
                                this_request->report_exception(std::current_exception());
                ...
                {
                    writeBuffer.putn_nocopy(asio::buffer_cast<const uint8_t *>(m_body_buf.data()), to_read).then([this_request, to_read](pplx::task<size_t> op)
                    {
                        try
                        {
                            op.wait();
    ...
    void handle_read_content(const std::error_code& ec)
    {
        ...
        if (m_downloaded < m_content_length)
        {
            ...
            if(m_decompressor)
            {
                auto decompressed = m_decompressor->decompress(asio::buffer_cast<const uint8_t *>(m_body_buf.data()), read_size);
                ...
                if (decompressed.empty())
                {
                    try
                    {
                        ...
                        this_request->async_read_until_buffersize(static_cast<size_t>(std::min(static_cast<uint64_t>(this_request->m_http_client->client_config().chunksize()), this_request->m_content_length - this_request->m_downloaded)),
                            std::bind(&asio_context::handle_read_content, this_request, std::placeholders::_1 /*asio::placeholders::error*/));
                    }
                    catch (...)
                    {
                        this_request->report_exception(std::current_exception());
    ...
    public:
        ...
        void start()
        {
            ...
            auto ctx = m_ctx;
            m_timer.async_wait([ctx](const std::error_code& ec)
                               {
        ...
        static void handle_timeout(const std::error_code& ec,
